...
import {Autocomplete as AA } from '@mantine/core'
...
const searchFun = (
  ...
  axios
    .get('https://nominatim.openstreetmap.org/search?format=geojson&limit=20&q=' + queryParam)
    .then(({data}) => {
      console.log(data.features)
      setIsLoading(false);
      setResults(data.features ? data.features.map((i: any) => i.properties.display_name) : []);
    });
};

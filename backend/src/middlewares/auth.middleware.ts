...
export const authMiddleware = {
  required: async (req, res, next) => {
    try {
      ...
      if (isNullOrUndefined(accessToken)) return next(new ApiError(401, { message: 'User is unauthorized!' }))

      const userData: any = validateAccessToken(accessToken)

      if (isNullOrUndefined(userData)) return next(new ApiError(401, { message: 'User is unauthorized!' }))
  ...
  optional: async (req, res, next) => {
    ...
    if (!isNullOrUndefined(accessToken) && accessToken != 'null') {
      const userData: any = validateAccessToken(accessToken)

      if (!isNullOrUndefined(userData)) req.user = await getCurrentUser(Number(userData.id))
    } else {
      req.user = {
        id: -1,
        role: 'GUEST'
...
export const roleCheckMiddleware = (roles: any) => {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (!roles.includes(currentUser.role)) return next(new ApiError(403, { message: 'User is unauthorized!' }))
